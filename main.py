...
def home(response: Response, request: Request):
    # Cookieのチェックを削除し、常にhome.htmlを返す
...
def channel(channelid: str, response: Response, request: Request):
    ...
    return template("channel.html", {"request": request, "results": t[0], "channel_name": t[1]["channel_name"], "channel_icon": t[1]["channel_icon"], "channel_profile": t[1]["channel_profile"], "cover_img_url": t[1]["author_banner"], "subscribers_count": t[1]["subscribers_count"]})
...
def bbs(request: Request, name: Union[str, None] = "", seed: Union[str, None] = "", channel: Union[str, None] = "main", verify: Union[str, None] = "false"):
...
def view_commonds(request: Request):
